...
def load_data(len1):
    model = open('sgns.financial.word', 'r',encoding='utf-8')
    ...
    for eachword in model:
        eachword.replace(' \n','')
        list1 = eachword.split(' ')
        if len(eachword) < 300:
            continue
        for i in range(1,301):
            list1[i] = float(list1[i])
        dict_vec[list1[0]] = list1[1:301]
...
if __name__=='__main__':
    ...
    model = Sequential()
    model.add(LSTM(input_dim=300, output_dim=30, return_sequences=False))
